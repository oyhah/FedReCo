...
class Client(DistributedTrainingDevice):
    ...
    def __init__(self, model, args, trn_x, trn_y, tst_x, tst_y, n_cls, dataset_name, id_num=0):
        ...
        self.trn_whole = DataLoader(Dataset(trn_x, trn_y, train=True, dataset_name=dataset_name),
    ...
    def train_cnn_FedReCo(self, w_glob_keys, server, last):

        self.model.train()

        params_first = [param for name, param in self.model.named_parameters() if name in w_glob_keys]
        params_last = [param for name, param in self.model.named_parameters() if name not in w_glob_keys]

        optimizer = torch.optim.SGD([
            {'params': params_first, 'lr': self.args.lr_first},
            {'params': params_last, 'lr': self.args.lr_last}
        ], momentum=self.args.momentum, weight_decay=self.args.weigh_delay)
        ...
        last_eps = local_eps - self.args.local_rep_ep

        for iter in range(last_eps):
        ...
        for iter in range(self.args.local_rep_ep):
        # Then do local epochs for the representation
            ...
            for i in range(self.local_epoch):
                ...
                optimizer.zero_grad()
                rep, log_probs = self.model(images)
                rep_glo, _ = server.model(images)
                loss_f_i = self.loss_func(log_probs, labels.reshape(-1).long())
                loss_h_i = torch.square(torch.norm(rep - rep_glo))
                
                loss = loss_f_i + self.args.lambda_value * loss_h_i / 2
    ...
    def compute_weight_update(self, w_glob_keys, server, last=False):
        ...
        # W = SGD(W, D)
        if self.args.method == 'FedReCo':
            self.train_loss = self.train_cnn_FedReCo(w_glob_keys, server, last)
        else:
            self.train_loss = self.train_cnn(w_glob_keys, server, last)

    def global_gradient(self, w_glob_keys, server):

        optimizer = torch.optim.SGD(server.model.parameters(), lr=self.args.lr_server, momentum=self.args.momentum,
                                        weight_decay=self.args.weigh_delay)
        ...
        trn_whole_iter = self.trn_whole.__iter__()
        images, labels = trn_whole_iter.__next__()
        images, labels = images.to(self.args.device), labels.to(self.args.device)

        optimizer.zero_grad()
        rep, _ = self.model(images)
        rep_glo, _ = server.model(images)
        loss_h_i = torch.square(torch.norm(rep - rep_glo))
        ...
        h_gradient = {name: param.grad for name, param in server.model.named_parameters() if name in w_glob_keys}
    ...
    @torch.no_grad()
    def evaluate_FedReCo(self, data_x, data_y, dataset_name):
        self.model.eval()
        # testing
        test_loss = 0
        acc_overall = 0
...
class Server(DistributedTrainingDevice):
    ...
    def first_part_update(self, h_gradient_total, w_glob_keys):
        # Update the first part of globel model at FedReCo
        self.model.train()

        params_first = [param for name, param in self.model.named_parameters() if name in w_glob_keys]
        params_first_name = {name: param for name, param in self.model.named_parameters() if name in w_glob_keys}

        optimizer = torch.optim.SGD(params_first, lr=self.args.lr_server, momentum=self.args.momentum,
                                    weight_decay=self.args.weigh_delay)
        scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=1, gamma=1)
        ...
        for name in w_glob_keys:
            params_first_name[name].grad.data = torch.mean(torch.stack([h_gradient[name] for h_gradient in h_gradient_total]), dim=0).clone()
